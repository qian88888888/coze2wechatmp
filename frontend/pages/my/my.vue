<template>
	<view>
		...
		<view @click="login" class="myUserInfo" v-if="!token">
			...
			<view class="portrait">
				<image :src="cdnUrl+'/bbw.png'"></image>
			</view>
		</view>
		...
		<view class="my-item-list">
			...
			<view v-if="token" class="item-cart" @click="history">
				<image :src="cdnUrl+'/ls.png?1'"></image>
				<view class="cart-text">创作历史</view>
			</view>
			<view v-if="token" class="item-cart" @click="document">
				<image :src="cdnUrl+'/wd.png'"></image>
				<view class="cart-text">我的文档</view>
			</view>
			
		
			<view v-if="token" class="item-cart" @click="loginout">
				<image :src="cdnUrl+'/logout.png'"></image>
...
<script>
	...
	export default {
		...
		methods: {
			...
			assistantList(){
				uni.navigateTo({
					url: '/pages/assistant/index'+'?type=1'
				});
			},
			vip(){
				uni.navigateTo({
